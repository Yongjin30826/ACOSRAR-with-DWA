...
function xyz1 = PlotSolution_new(sol,model,smooth)
...
for i = 1:N
    z_map = model.H(round(y_all(i)),round(x_all(i)));    %  寻找最相似的值，（x，y）对应的纵坐标
...
for k=1:ndim     % 使用样条插值平滑地估计一个三维点集的坐标
    xyzp(k,:)=ppval(csaps(1:npts,xyz(k,:),smooth),1:npts);
end
xyz1 = xyzp;
plot3(xyzp(1,:),xyzp(2,:),xyzp(3,:),'k--','LineWidth',1);
